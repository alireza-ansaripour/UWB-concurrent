...
//
  ...
  if(HAL_UART_Receive_IT(&USARTx_Handle, (uint8_t *)UserTxBuffer, 1) != HAL_OK)
  {
  ...
  USBD_CDC_SetTxBuffer(&USBD_Handle, UserTxBuffer, 0);
  USBD_CDC_SetRxBuffer(&USBD_Handle, UserRxBuffer);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if(UserTxBufPtrOut != UserTxBufPtrIn)
  {
    ...
    USBD_CDC_SetTxBuffer(&USBD_Handle, (uint8_t*)&UserTxBuffer[buffptr], buffsize);
    
    if(USBD_CDC_TransmitPacket(&USBD_Handle) == USBD_OK)
    {
...
{
  HAL_UART_Transmit_DMA(&USARTx_Handle, Buf, *Len);
  return (USBD_OK);
}
...
{  
  ...
  */
  TIMx_Handle.Init.Period = (CDC_POLLING_INTERVAL*1000) - 1;
  TIMx_Handle.Init.Prescaler = 84-1;
  TIMx_Handle.Init.ClockDivision = 0;
